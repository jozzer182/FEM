...
class FechasFEM {
  ...
  Future<List<FechasFEMSingle>> obtener() async {
    Map<String, Object> dataSend = {
      'dataReq': {'libro': 'FECHAS_FEM', 'hoja': 'reg'},
      'fname': "getHojaList"
    };
    ...
    for (var item in dataAsListMap.sublist(1)) {
      fechasFEMList.add(FechasFEMSingle.fromList(item));
      fechasFemDateBoolList.add(FechasFEMDateBool.fromList(item));
    }
    for (FechasFEMDateBool fecha in fechasFemDateBoolList) {
      if (fechasFEMAsYearList.indexWhere((e) => e.ano == fecha.ano) == -1) {
        fechasFEMAsYearList.add(FechasFEMAsYear.fromInit(fecha.ano));
      }
      FechasFEMAsYear fechasFEMAsYear =
          fechasFEMAsYearList.firstWhere((e) => e.ano == fecha.ano);
      fechasFEMAsYear.add(fecha);
    }
    // print('fechasFEMAsYearList ${fechasFEMAsYearList}');
    fechasFEMListSearch = [...fechasFEMList];
  ...
  Map<int, EnableDateInt> enableDatesInt(int ano) {
    ...
    List<FechasFEMSingle> anoList =
        fechasFEMList.where((e) => e.ano == '$ano').toList();
    for (int mes in meses) {
      List<FechasFEMSingle> mesEstado = anoList
...
class FechasFEMSingle {
  ...
  factory FechasFEMSingle.fromList(List l) {
    return FechasFEMSingle(
      ...
      name: l[2].toString(),
      pedido: l[3].toString(),
      fecha: l[4].toString().length > 10
          ? l[4].toString().substring(0, 10)
          : l[4].toString(),
      estado: l[5].toString(),
      fechadelivery: l[6].toString().length > 10
          ? l[6].toString().substring(0, 10)
          : l[6].toString(),
      delivered: l[7].toString(),
      versionfecha: l[8].toString().length > 10
