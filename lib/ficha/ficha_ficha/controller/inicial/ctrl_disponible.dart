...
class CtrlFfichaInicialDisponibilidad {
  ...
  void _evaluarColorCeldaNuevo(
    ...
    bool agendadoActivo = fichaRegMod.agendado.activoMes.get(mes);
    // int planificacionNueva = fichaRegMod.planificado.mes.getInt(mes);

    List<String> mesesActivos = fichaRegMod.version.mesesActivos;
    int planificacionActiva = mesesActivos
        .map((e) => fichaRegMod.planificado.mes.getInt(e))
        .fold(0, (a, b) => a + b);

    if (!versionActiva && agendadoActivo && planificacionActiva > 0) {
      if (disponibilidadNew < 0) {
        fichaRegMod.error.setError(
          mes: mes,
          error: 'Se ha superado el límite de disponibilidad',
        );
  ...
  void _evaluarColorCeldaViejo(
    ...
    bool agendadoActivo = fichaRegMod.agendado.activoMes.get(mes);
    List<String> mesesActivos = fichaRegMod.version.mesesActivos;
    int planificacionActivaNueva = mesesActivos
        .map((e) => fichaRegMod.planificado.mes.getInt(e))
        .fold(0, (a, b) => a + b);
    if (!versionActiva && agendadoActivo) {
      FichaReg fichaRegOld =
          state().ficha!.fficha.ficha.firstWhere((e) => e.id == fichaRegMod.id);
      
      int planificacionActivaVieja = mesesActivos
          .map((e) => fichaRegOld.planificado.mes.getInt(e))
          .fold(0, (a, b) => a + b);

      // int planificacionNueva = fichaRegMod.planificado.mes.getInt(mes);

      if (disponibilidadNew < 0 && planificacionActivaNueva > planificacionActivaVieja) {
        // List<DisponibilidadSingle> dispList = state()
        //     .disponibilidad!
        //     .disponibilidadList
        //     .where(
        //       (disponibilidad) => disponibilidad.e4e.contains(fichaRegMod.e4e),
        //     )
        //     .toList();
        // DisponibilidadSingle disp = dispList.first;
        // String totalDisponible = disp.total;
        // String mesDisponible = disp.mesFin;
